...
class Aircraft(Model): #Overall vehicle model 
        ...
        def setup(self):
                ...
                Swet = Variable("Swet","m^2","Aircraft wetted area")
                ...
                return [self.components, 
                        W_TO >= W_ZF + W_fuel,
                        W_ZF >= sum(c["W"] for c in self.components),
                        Swet >= sum(c["Swet"] for c in self.components)]
...
class Engines(Model): #Fixed engine only for now
        def setup(self):
                ...
                return [W == g * num_engines * m_engine]
